...
class ChannelStatusWidget(tk.Frame):
    def __init__(self, master, creator, *args, **kwargs):
        ...
        self.UID = creator.UID
        self.name = creator.nickname
        self.platform = creator.main_platform
        self.chzzk_url = creator.chzzk_url
        self.soop_url = creator.soop_url
        self.fan_cafe_url = creator.fan_cafe
        self.youtube_url = creator.youtube
        self.twitter_url = creator.twitter_url

        self.label_name = tk.Label(self, text=self.name, bg="#ffffff", fg="black", cursor="hand2", font=self.font_normal)
        self.label_platform = tk.Label(self, text=self.platform, bg="#ffffff", fg="black", cursor="hand2", font=self.font_normal)
        self.label_fan_cafe = tk.Label(self, text="팬카페", bg="#ffffff", fg="black", cursor="hand2", font=self.font_normal)
        self.label_youtube = tk.Label(self, text="유튜브", bg="#ffffff", fg="black", cursor="hand2", font=self.font_normal)
        self.label_twitter = tk.Label(self, text="트위터", bg="#ffffff", fg="black", cursor="hand2", font=self.font_normal)
        #self.label_twitter = tk.Label(self, text="트위터", bg="#ffffff", fg="blue", cursor="hand2", underline=True)#하이퍼링크처럼보이게하기
